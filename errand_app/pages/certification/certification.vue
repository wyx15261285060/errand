<template>
	<view style="padding: 20rpx;">
		<view class="box">
			<uni-forms :modelValue="form" :rules="rules" ref="formRef" label-width="220rpx" label-align="right"
				...
				<uni-forms-item label="身份证正面" name="card1" required>
					<uni-file-picker limit="1" :image-styles="imageStyles" :del-icon="false" :disable-preview="true"
						fileMediatype="image" v-model="cardImgs1" @select="handleCard1UploadSuccess"></uni-file-picker>
				</uni-forms-item>
				<uni-forms-item label="身份证反面" name="card2" required>
					<uni-file-picker limit="1" :image-styles="imageStyles" :del-icon="false" :disable-preview="true"
						fileMediatype="image" v-model="cardImgs2" @select="handleCard2UploadSuccess"></uni-file-picker>
				</uni-forms-item>
				...
				<view style="margin-bottom: 20rpx;">
					<button :disabled="form.status" class="button" type="primary" @click="submitRequest">提交申请</button>
				</view>
				<view v-if="form.id && form.status === '拒绝'">
					<button class="button-del" type="primary" @click="deleteRequest">删除申请</button>
				</view>
			</uni-forms>
		</view>
	</view>
...
<script>
	export default {
		...
		methods: {
			// 查询出当前用户的认证信息
			load() {
				this.$request.get('/certification/selectUserCert').then(res => {
					this.form = res.data || {}
					if (this.form.avatar) {
						this.imgs.url = this.form.avatar
					}
					if (this.form.card1) {
						this.cardImgs1.url = this.form.card1
					}
					if (this.form.card2) {
